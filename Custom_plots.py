...
def model_Confmat(model, X, Y, 
    ...
    pred_labels = [Label_Mapping[idx] for idx in preds.argmax(axis=1)]
    return confusion_matrix(y_true=Y, y_pred=pred_labels, labels=Labels)

def PlotModelEval(Model, History, X, Y, Label_Mapping, Labels):
    ...
    P  = [Label_Mapping[idx] for idx in S.argmax(axis=1)]
    # Calculate confusion matrix
    CM = model_Confmat(Model, X, Y, Label_Mapping,Labels)
    ...
    plt.subplot(2,2,3)
    plt.plot(100 * np.array(History.history['categorical_accuracy']), label="Training")
    if 'val_categorical_accuracy' in History.history:
        plt.plot(100 * np.array(History.history['val_categorical_accuracy']), label="Validation")
